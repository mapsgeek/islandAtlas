<template>
	<div>
		<div style="margin-bottom: 30px;" v-for="group in maps" :key="group.id">
			...
			<a-collapse :bordered="false" v-for="category in group.categorysByGroupsId" :key="category.id.toString()"
				expand-icon-position="right" v-model="activeKey" @change="changedKey">
				...
				<a-collapse-panel :key="category.id.toString()" :header="category.name" :style="customStyle" :forceRender="true">
					<div class="map" v-for="map in category.mapsByCategoryId" :key="map.id">
						<div class="previewMap">
							<img v-if="map.island !=='pohnpei'" :src="`${url}/${island}/${theme}/preview/thumbnails/${map.fileName}.png`"
								alt="thumbnail">
							<img v-if="map.island==='pohnpei'" :src="`${url}/${island}/${theme}/preview/thumbnails/${map.fileName}.jpg`"
								alt="thumbnail">
						</div>
						<div class="content">
